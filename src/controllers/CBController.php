...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function getPrint(){
        if(g('t_key')){
            $template = DB::table('cms_printout_templates')->where('key',g('t_key'))->first();
            if(!$template) CRUDBooster::redirectBack('Invalid Template','danger');
            try {
                // dd($template);
                if($template->file_type == "Excel" && $template->type == "Batch Data"){
                    $templateString = new $template->export_php;
                    $fileName = $template->file_name.date("YmdHis").".xlsx";
                    // dd($templateString);
                    return Excel::download($templateString, $fileName);
                }
            } catch(\Exception $e) {
                dd($e->getMessage());
            }
        } else {
            CRUDBooster::redirectBack('Invalid Template','danger');
        }
    }
